...
def set_determinism(
    ...
    if seed is None:
        ...
        # MODIFYING SEED TO REMAIN WITHIN A VALID RANGE
        # Convert the tensor to a Python integer safely
        seed = seed_tensor.to("cpu").view(torch.uint64).item()

        # Clamp the seed to ensure it fits within a valid range
        seed = seed % (2**32)  # Restrict seed to 32-bit range
